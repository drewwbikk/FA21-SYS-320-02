...
function block_menu() {
	...
	case "$choice" in
		...
		C|c) bash parse-threat-hw.bash -c
		;;
		D|d) bash parse-threat-hw.bash -u
		;;
		W|w) bash parse-threat-hw.bash -w
		;;
		I|i) bash parse-threat-hw.bash -i
		;;
		M|m) bash parse-threat-hw.bash -m
		;;
		S|s) security_menu
		;;
		E|e) exit 0
